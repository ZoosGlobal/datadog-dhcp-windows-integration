...
#              5. Unblocks both scripts (removes Zone.Identifier from downloads)
...
$TargetScript  = "$ScriptsDir\dhcp-metrics.ps1"
...
# Files downloaded from internet/GitHub are automatically blocked by Windows.
# Unblock-File removes the hidden Zone.Identifier NTFS stream so scripts run.
# ==============================================================================
Write-Step "5/7" "Unblocking scripts (removing Zone.Identifier)..."

try {
    # Unblock the copied target script
    Unblock-File -Path $TargetScript -ErrorAction SilentlyContinue
    Write-OK "Unblocked: $TargetScript"

    # Also unblock setup.ps1 itself in case it was re-run from a new location
    ...
    # Also unblock the source script in current directory if still present
    $sourcePath = Join-Path (Get-Location).Path $SourceScript
...
try {
    # Remove existing task if present (clean re-deploy)
